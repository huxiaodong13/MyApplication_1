...
public class BorrowCardUtil extends AppCompatActivity {
    private Context _Context;
    private DbHelper helper;
    public BorrowCardUtil(Context _Context)
    {
        try {
    ...
    public long Add(HashMap<String,String> account)
    {
        ...
        value.put(DbHelper.KEY_REGISTERDATE, account.get("register_date"));
        value.put(DbHelper.KEY_PHONE, account.get("phone"));
        long count=0;
        count=helper.insert(DbHelper.DB_TABLE4, DbHelper.KEY_ID, value);
        return count;
    }
    //删除
    ...
    public ArrayList<HashMap<String,String>> query()
    {
        ...
        Cursor c = helper.query(DbHelper.DB_TABLE4,null,null,null,null,null,null);
        while(c.moveToNext())
        {
            ...
            item.put("register_date",c.getString(c.getColumnIndex("register_date")));
            item.put("phone",c.getString(c.getColumnIndex("phone")));
    ...
    public ArrayList<HashMap<String,String>> query2(String text)
    {
        ...
        ArrayList<HashMap<String,String>> alUser=new ArrayList<HashMap<String,String>>();
        Cursor c = helper.query(DbHelper.DB_TABLE4,null,DbHelper.KEY_BORROWID+"=?",new String[]{text},null,null,null);
        while(c.moveToNext())
        {
            HashMap<String,String> item = new HashMap<String,String>();
            item.put("borrow_id", c.getString(c.getColumnIndex("borrow_id")));
            item.put("word_id", c.getString(c.getColumnIndex("word_id")));
            alUser.add(item);
        }
        c.close();
        c=null;
